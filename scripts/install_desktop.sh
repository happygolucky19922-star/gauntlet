...
DEFAULT_PROJECT_DIR="${PROJECT_DIR:-$(cd "${SCRIPT_DIR}/.." && pwd)}"
...
exec "\${PROJECT_DIR}/scripts/run_backend.sh"
LAUNCH
chmod +x "${BIN_PATH}"

if [[ ! -f "${ICON_PATH}" ]]; then
  # lightweight embedded icon (1x1 transparent png base64)
...
echo "Project path: ${DEFAULT_PROJECT_DIR}"
